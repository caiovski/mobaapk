-- =============================================================
-- Migration 39: Add confirmed_at timestamp to orders table
-- =============================================================
-- Adds confirmed_at column to track when the order was confirmed
-- (processing→confirmed transition by admin or PIX payment).
-- For non-PIX orders, this is set at creation time.
-- =============================================================
...
-- 2. Update update_order_status RPC: set confirmed_at on processing→confirmed
...
BEGIN
  ...
  SET status = p_new_status::order_status,
      ...
      confirmed_at = CASE
        WHEN v_current_status = 'processing' AND p_new_status = 'confirmed' THEN timezone('utc'::text, now())
        ELSE confirmed_at
      END,
      prepared_at = CASE
...
-- 3. Update confirm_pix_payment: set confirmed_at on processing→confirmed
...
-- 4. Update finalizar_pedido: set confirmed_at for non-PIX orders
